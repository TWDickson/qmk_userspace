...
Why this keymap is the way it is: the corrections, the things that look
redundant and are not, and the traps. **[README.md](README.md) is the reference
for what the board actually does** — layers, themes, controls. This file is the
reasoning behind it, and is written for whoever next has to change something.

Most of it exists because a previous version got the thing wrong, so the entries
tend to be phrased as "this used to be X, and here is why X was untrue."
